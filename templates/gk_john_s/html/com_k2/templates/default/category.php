...
<div id="k2Container" class="itemListView<?php if($this->params->get('pageclass_sfx')) echo ' '.$this->params->get('pageclass_sfx'); ?> no-bg">
		...
		<div class="itemList">
				...
				<div id="itemListLeading">
						<?php for($i = 0; $i < $this->params->get('num_leading_columns'); $i++) : ?>
							<div class="itemContainer"<?php echo (count($this->leading)==1) ? '' : ' style="width:'.(number_format(100/$this->params->get('num_leading_columns'), 1)-0.1).'%;"'; ?>>
							<?php foreach($this->leading as $key=>$item): ?>
								<?php if($key % $this->params->get('num_leading_columns') == $i) : ?>
								<?php
									$this->item=$item;
				...
				<div id="itemListPrimary">
						<?php for($i = 0; $i < $this->params->get('num_primary_columns'); $i++) : ?>
							<div class="itemContainer"<?php echo (count($this->primary)==1) ? '' : ' style="width:'.(number_format(100/$this->params->get('num_primary_columns'), 1)-0.1).'%;"'; ?>>
							<?php foreach($this->primary as $key=>$item): ?>
								<?php if($key % $this->params->get('num_primary_columns') == $i) : ?>
								<?php
									$this->item=$item;
									echo $this->loadTemplate('item');
								?>
				...
				<div id="itemListSecondary">
						<?php for($i = 0; $i < $this->params->get('num_secondary_columns'); $i++) : ?>
							<div class="itemContainer"<?php echo (count($this->secondary)==1) ? '' : ' style="width:'.(number_format(100/$this->params->get('num_secondary_columns'), 1)-0.1).'%;"'; ?>>
							<?php foreach($this->secondary as $key=>$item): ?>
								<?php if($key % $this->params->get('num_secondary_columns') == $i) : ?>
								<?php
									$this->item=$item;
									echo $this->loadTemplate('item');
								?>
